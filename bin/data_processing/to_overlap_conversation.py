...
def generate_train_dataset_for_pretrain(length: int=2):
    ...
    jsonl_dir = f"/n/work6/yizhang/Moris/zoom2025/pretrain_labels/{split}"
    output_dir = f"/n/work6/yizhang/Moris/zoom2025/pretrain_labels/l{length}_conv_{split}"
...
def generate_train_dataset_for_finetune(length: int=2):
    ...
    output_dir = f"/n/work6/yizhang/Moris/zoom2025/finetune_labels/l{length}_conv_{split}"
    Path(output_dir).mkdir(exist_ok=True, parents=True)
    generate_dataset(jsonl_dir=jsonl_dir, output_dir=output_dir, length=length)
