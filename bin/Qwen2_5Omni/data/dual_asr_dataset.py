...
def get_transcript_before_time(
    ...
    while i < len(stream) and stream[i]["end"] <= t_end + eps:
        ...
        if kind == "asr":
            ...
            # 这里假设 bc 是独立事件 token，例如 {"kind": "bc", "token": "<bc>"}
            # 不把它包在 speaker tag 里。
            close_speaker()
...
class DualChannelStreamingASRDataset(Dataset):
    """
...
if __name__ == "__main__":
    ...
    dir = "/n/work6/yizhang/Moris/zoom2025/pretrain_labels/l4_conv_train"
    ...
    ds = DualChannelStreamingASRDataset(
