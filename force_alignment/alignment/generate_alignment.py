...
class Alignment:
    def __init__(self, 
        ...
        self.model, self.metadata = self.load_align_model(language_code, device, model_name_or_path, model_dir)
        
        transcript_dataset = TranscriptDatasetForTXT(annotation_dir=annotation_dir, target_sr=target_sr)
    ...
    def load_align_model(self, language_code: str, device: str, model_name_or_path: Optional[str] = None, model_dir=None):
        ...
        if model_name_or_path in torchaudio.pipelines.__all__:
            ...
            try:
                processor = Wav2Vec2Processor.from_pretrained(model_name_or_path, cache_dir=model_dir, trust_remote_code=True)
                align_model = Wav2Vec2ForCTC.from_pretrained(model_name_or_path, cache_dir=model_dir, trust_remote_code=True)
            except Exception as e:
                print(e)
                print(f"Error loading model from huggingface, check https://huggingface.co/models for finetuned wav2vec2.0 models")
                raise ValueError(f'The chosen align_model "{model_name_or_path}" could not be found in huggingface (https://huggingface.co/models) or torchaudio (https://pytorch.org/audio/stable/pipelines.html#id14)')
    ...
    def align(
        ...
        for sdx, segment in enumerate(transcript):
            ...
            if path is None:
                ...
                aligned_seg["words"] = [{"word": text, "start": aligned_seg["start"], "end": aligned_seg["end"]}]
                aligned_segments.append(aligned_seg)
                continue
    ...
    def save_to_jsonl(segments: list, audio_path: str, output_path: str):
        ...
        with output_path.open("w", encoding="utf-8") as f:
            ...
            print(f"Saved {len(segments)} aligned segments to {output_path}")
    
    def generate(self):
        for batch in self.transcript_loader:
            waveforms    = batch["waveforms"]           # [T_total]
            segments     = batch["segments"]      # List[SingleSegment]
            ...
            for waveform, segment, audio_path in zip(waveforms, segments, audio_paths):
                ...
                stem = pathlib.Path(audio_path).parent.stem + "_" + pathlib.Path(audio_path).stem
...
def add_arguments(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        ...
        type=str, 
        default=None, 
        help="Alignment model to use (if not specified, will use default model for the language if available)"
    )
    ...
    parser.add_argument(
        ...
        type=str, 
        default=None, 
        help="Directory to cache alignment models (default: None)"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="cuda",
        help="Device to run alignment on (e.g. 'cuda' or 'cpu')"
    )
